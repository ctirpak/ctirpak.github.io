...
model.bio.display = function () {
	...
	var formattedContact = HTMLcontactGeneric.replace("%data%", model.bio.contacts.email);
	...
	var formattedLinkedIn = HTMLlinkedIn.replace("%data%", model.bio.contacts.LinkedIn);
	...
	var bioPic = HTMLbioPic.replace("%data%", model.bio.pictureURL);
...
model.education.display = function () {
	...
	for (school in model.education.schools) {
		...
		formattedSchool = HTMLschoolName.replace("%data%", model.education.schools[school].name);
		formattedDegree = HTMLschoolDegree.replace("%data%", model.education.schools[school].Degree);
		formattedDates = HTMLschoolDates.replace("%data%", model.education.schools[school].years);
		formattedLocation = HTMLschoolLocation.replace("%data%", model.education.schools[school].location);
		formattedDescription = HTMLschoolDescription.replace("%data%", model.education.schools[school].description);
...
model.work.display = function () {
	for (job in model.work.jobs) {
		...
		var formattedPosition = HTMLworkTitle.replace("%data%", model.work.jobs[job].position);
		var formattedEmployerPosition = formattedEmployer + formattedPosition;
		var formattedDates = HTMLworkDates.replace("%data%", model.work.jobs[job].years);
...
model.projects.display = function () {
	if (model.projects.project.length > 0) {
		var formattedSkill;
		...
		for (proj in model.projects.project) {
			formattedProjTitle = HTMLprojectTitle.replace("%data%", model.projects.project[proj].title);
			formattedProjDates = HTMLprojectDates.replace("%data%", model.projects.project[proj].dates);
			formattedProjDesc = HTMLprojectDescription.replace("%data%", model.projects.project[proj].description);

			$(".project-entry:last").append(formattedProjTitle);
			$(".project-entry:last").append(formattedProjDates);
			$(".project-entry:last").append(formattedProjDesc);

			for (img in model.projects.project[proj].images) {
				formattedProjImg = HTMLprojectImage.replace("%data%", model.projects.project[proj].images[img]);
